...
export function registerUnifiedTools(server, kernel) {
  ...
  tool('chat_tasks', 'List local task metadata without opening a browser or exposing prompt text.', { provider, limit: z.number().int().min(1).max(100).default(20) }, (i) => kernel.list(i.provider, i.limit));
